...
def rmsle_scorer(y_true, y_pred):
    """Scorer для использования в RandomizedSearchCV"""
...
def main():
    ...
    results_df.to_csv('hyperparameter_tuning_random_forest_results.csv', index=False)

    # Сохраняем все результаты CV в детальный файл
    cv_results = pd.DataFrame(random_search.cv_results_)
    cv_results.to_csv('random_search_random_forest_cv_results.csv', index=False)
